% findCondYawVelPeaksFT2P.m
%
% Helper function for saveDFFCond_bouts() that takes in fwdVelCond,
%  fictracSmo, and cond and returns times for valid yaw
%  velocity peaks as well as start/end times for corresponding bouts.
% Boolean for whether to extract right or left turns
% 
% Adapted from findCondYawVelPeaksFT(), which operates on ephys/opto data.
...
%   cond - pass from saveDFFCond_bouts()
%   fwdVelCond - pass from saveDFFCond_bouts()
%
% OUTPUTS:
...
%     fictracSmo.slideVel = fictracSmo.slideVel - fictracSmo.slideVelBias;
    ...
    % get indices of not moving times
    notMoveLog = ~fictracSmo.moveLog;
    notMoveInd = find(notMoveLog);

    % remove peaks during not moving times
    pkInds = setdiff(pkInds, notMoveInd,'stable');
