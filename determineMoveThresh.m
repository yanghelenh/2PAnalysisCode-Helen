...
%     moveStartTimes = t((diff(moveLogical) > 0.9) + 1);
    ...
    for i = 1:length(moveStartInd)
        if ~isempty(find(fictrac.dropInd==moveStartInd(i),1))
    ...
    for i = 1:length(moveEndInd)
        if ~isempty(find(fictrac.dropInd==moveEndInd(i),1))
            deleteInd = [deleteInd i];
        end
    end
    ...
    fictrac.moveParams.thresh = thresh;
    fictrac.moveParams.sigma = sigmaVel;
    fictrac.moveParams.minBoutLen = minBoutLen;
    fictrac.moveLog = moveLogical;
    fictrac.totSpdNormSmo = totSpdNormSmo;
    fictrac.moveStartInd = moveStartInd;
    fictrac.moveEndInd = moveEndInd;
    
    % note that moveStartTimes and moveEndTimes account for dropInd and
    %  ignore any times that fall during this time period
    fictrac.moveStartTimes = moveStartTimes;
    fictrac.moveEndTimes = moveEndTimes;
    
    % update pData
    save(pdFile, 'fictrac', '-append');
